(function () {
    ...
    async function fetchEvents() {
        ...
        try {
            ...
            if (tableBody) {
                tableBody.innerHTML = `
        <tr>
          <td colspan="5" class="error">ไม่สามารถโหลดข้อมูลได้</td>
        </tr>
      `;
            }
            emptyState?.classList.remove('show');
    ...
    function renderTable(list) {
        ...
        <td data-label="วันเริ่ม">${formatDateTime(ev.start_date)}</td>
        <td data-label="สถานะ">
    ...
    newEventBtn?.addEventListener('click', () => {
        window.location.href = './event_manage.html?mode=create';
    });
