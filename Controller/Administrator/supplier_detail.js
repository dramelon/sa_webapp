(function () {
    ...
    const heading = document.getElementById('supplierHeading');
    const codeField = document.getElementById('supplierCode');
    const refField = document.getElementById('supplierRefCode');
    const nameField = document.getElementById('supplierName');
    const orgField = document.getElementById('supplierOrg');
    ...
    const taxField = document.getElementById('supplierTax');
    ...
    const metaRefs = {
        id: document.getElementById('supplierIdDisplay'),
        ref: document.getElementById('supplierRef'),
        statusMeta: document.getElementById('supplierStatusMeta'),
        statusBadge: document.getElementById('supplierStatusBadge'),
        statusText: document.getElementById('supplierStatusText'),
        createdAt: document.getElementById('supplierCreatedAt'),
        createdBy: document.getElementById('supplierCreatedBy'),
        updatedAt: document.getElementById('supplierUpdatedAt'),
        updatedBy: document.getElementById('supplierUpdatedBy'),
    };
    ...
    let currentSupplierId = supplierIdParam ? String(supplierIdParam) : null;
    let isCreateMode = !currentSupplierId || isCreateRequested;
    let modelRoot = '';
    let isDirty = false;
    let isSaving = false;
    ...
    function serializeForm() {
        return {
            ref_supplier_id: refField?.value.trim() || '',
            supplier_name: nameField?.value.trim() || '',
    ...
    function updateMeta(data) {
        if (metaRefs.id) {
            metaRefs.id.textContent = data?.supplier_id != null ? String(data.supplier_id) : '—';
        }
        if (metaRefs.ref) {
            const refText = data?.ref_supplier_id ? data.ref_supplier_id : '—';
    ...
    function populateForm(data) {
        const snapshot = {
            ref_supplier_id: data?.ref_supplier_id || '',
        ...
        runWithPopulation(() => {
            if (refField) refField.value = snapshot.ref_supplier_id;
            if (nameField) nameField.value = snapshot.supplier_name;
        ...
        updateMeta(data);
        if (heading) {
            heading.textContent = data?.supplier_name || (data?.supplier_id ? `ซัพพลายเออร์ #${data.supplier_id}` : 'รายละเอียดซัพพลายเออร์');
        }
        if (codeField) {
            codeField.value = data?.supplier_id != null ? String(data.supplier_id) : 'ใหม่';
        }
    ...
    function prepareCreateMode() {
        ...
        if (heading) {
            heading.textContent = 'สร้างซัพพลายเออร์ใหม่';
        }
        runWithPopulation(() => {
            form?.reset();
            if (codeField) {
    ...
    async function loadSupplier(id) {
        ...
        try {
            const response = await fetch(`${modelRoot}/supplier_detail.php?id=${encodeURIComponent(id)}`, {
                credentials: 'same-origin',
            });
            if (!response.ok) {
                if (response.status === 404) {
                    showMessage('ไม่พบข้อมูลซัพพลายเออร์', 'error');
                    prepareCreateMode();
                    return;
                }
            ...
            currentSupplierId = String(data.supplier_id);
            isCreateMode = false;
            populateForm(data);
        } catch (error) {
            showMessage('ไม่สามารถโหลดข้อมูลซัพพลายเออร์ได้', 'error');
            prepareCreateMode();
        }
    }
    ...
    async function createSupplier(payload) {
        const response = await fetch(`${modelRoot}/supplier_create.php`, {
            method: 'POST',
        ...
        if (!response.ok) {
            if (response.status === 409) {
                throw new Error('ref_supplier_exists');
            }
            throw new Error('network');
        }
    ...
    async function updateSupplier(payload) {
        const response = await fetch(`${modelRoot}/supplier_update.php`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            credentials: 'same-origin',
            body: JSON.stringify({ supplier_id: currentSupplierId, ...payload }),
        });
        if (!response.ok) {
            if (response.status === 409) {
                throw new Error('ref_supplier_exists');
    ...
    async function handleSubmit(event) {
        ...
        if (!nameValue) {
            showMessage('กรุณากรอกชื่อซัพพลายเออร์', 'error');
            nameField?.focus();
            return;
        }
        ...
        try {
            ...
            const data = wasCreate ? await createSupplier(payload) : await updateSupplier(payload);
            currentSupplierId = String(data.supplier_id);
            isCreateMode = false;
            params.set('supplier_id', currentSupplierId);
            params.delete('mode');
            const newQuery = params.toString();
            const newUrl = newQuery ? `${window.location.pathname}?${newQuery}` : window.location.pathname;
            window.history.replaceState({}, '', newUrl);
            populateForm(data);
            showMessage(wasCreate ? 'สร้างซัพพลายเออร์เรียบร้อยแล้ว' : 'บันทึกข้อมูลซัพพลายเออร์เรียบร้อยแล้ว', 'success');
            try {
                sessionStorage.setItem('suppliers:refresh', '1');
            } catch (storageError) {
            ...
            setDirtyState(true);
            if (error.message === 'ref_supplier_exists') {
                ...
                showMessage('ไม่สามารถบันทึกข้อมูลซัพพลายเออร์ได้ โปรดลองอีกครั้ง', 'error');
            }
        } finally {
            isSaving = false;
            updateSaveButtonState();
    ...
    function attachEventListeners() {
        ...
        if (breadcrumbLink) {
            breadcrumbLink.addEventListener('click', (event) => {
                event.preventDefault();
                const href = breadcrumbLink.getAttribute('href') || './suppliers.html';
                requestNavigation(() => {
    ...
    const boot = ({ root }) => {
        ...
        if (isCreateMode) {
            ...
            loadSupplier(currentSupplierId);
        }
    };
