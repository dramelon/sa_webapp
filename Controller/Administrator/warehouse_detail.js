(function () {
    ...
    const codeField = document.getElementById('warehouseCode');
    const nameField = document.getElementById('warehouseName');
    const serialField = document.getElementById('warehouseSerial');
    const statusField = document.getElementById('warehouseStatus');
    const noteField = document.getElementById('warehouseNote');
    ...
    const metaRefs = {
        id: document.getElementById('warehouseIdDisplay'),
        serial: document.getElementById('warehouseSerialMeta'),
        location: document.getElementById('warehouseLocationMeta'),
        statusBadge: document.getElementById('warehouseStatusBadge'),
        statusText: document.getElementById('warehouseStatusText'),
        createdAt: document.getElementById('warehouseCreatedAt'),
        createdBy: document.getElementById('warehouseCreatedBy'),
        updatedAt: document.getElementById('warehouseUpdatedAt'),
        updatedBy: document.getElementById('warehouseUpdatedBy'),
    };
    ...
    const warehouseIdParam = params.get('warehouse_id');
    const isCreateRequested = params.get('mode') === 'create';
    let currentWarehouseId = warehouseIdParam ? String(warehouseIdParam) : null;
    let isCreateMode = !currentWarehouseId || isCreateRequested;
    let modelRoot = '';
    let isDirty = false;
    let isSaving = false;
    ...
    function serializeForm() {
        return {
            warehouse_name: nameField?.value.trim() || '',
            warehouse_sn: serialField?.value.trim() || '',
    ...
    function updateMeta(data) {
        if (metaRefs.id) {
            metaRefs.id.textContent = data?.warehouse_id != null ? String(data.warehouse_id) : '—';
        }
        if (metaRefs.serial) {
            const serialText = data?.warehouse_sn ? data.warehouse_sn : '—';
            metaRefs.serial.textContent = `Serial: ${serialText}`;
        }
        if (metaRefs.location) {
            const locationText = data?.location_label || data?.location_name || '—';
            metaRefs.location.textContent = `สถานที่: ${locationText}`;
        }
    ...
    function populateForm(data) {
        const snapshot = {
            ...
            warehouse_sn: data?.warehouse_sn || '',
        ...
        runWithPopulation(() => {
            if (nameField) nameField.value = snapshot.warehouse_name;
            if (serialField) serialField.value = snapshot.warehouse_sn;
            if (statusField) statusField.value = snapshot.status;
            if (noteField) noteField.value = snapshot.note;
            if (locationLookup) {
        ...
        updateMeta({ ...data, location_label: snapshot.location_label });
        if (heading) {
            heading.textContent =
                data?.warehouse_name || (data?.warehouse_id ? `คลัง #${data.warehouse_id}` : 'รายละเอียดคลังสินค้า');
        }
        if (codeField) {
            codeField.value = data?.warehouse_id != null ? String(data.warehouse_id) : 'ใหม่';
        }
    ...
    function prepareCreateMode() {
        ...
        if (heading) {
            heading.textContent = 'สร้างคลังสินค้าใหม่';
        }
        runWithPopulation(() => {
            form?.reset();
            if (codeField) {
    ...
    async function loadWarehouse(id) {
        ...
        try {
            const response = await fetch(`${modelRoot}/warehouse_detail.php?id=${encodeURIComponent(id)}`, {
                credentials: 'same-origin',
            });
            if (!response.ok) {
                if (response.status === 404) {
                    showMessage('ไม่พบข้อมูลคลังสินค้า', 'error');
                    prepareCreateMode();
                    return;
                }
            ...
            currentWarehouseId = String(data.warehouse_id);
            isCreateMode = false;
            populateForm(data);
        } catch (error) {
            showMessage('ไม่สามารถโหลดข้อมูลคลังสินค้าได้', 'error');
            prepareCreateMode();
        }
    }
    ...
    async function createWarehouse(payload) {
        const response = await fetch(`${modelRoot}/warehouse_create.php`, {
            method: 'POST',
        ...
        if (!response.ok) {
            if (response.status === 409) {
                throw new Error('warehouse_sn_exists');
            }
            throw new Error('network');
        }
    ...
    async function updateWarehouse(payload) {
        const response = await fetch(`${modelRoot}/warehouse_update.php`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            credentials: 'same-origin',
            body: JSON.stringify({ warehouse_id: currentWarehouseId, ...payload }),
        });
        if (!response.ok) {
            if (response.status === 409) {
                throw new Error('warehouse_sn_exists');
    ...
    async function handleSubmit(event) {
        ...
        if (!nameValue) {
            showMessage('กรุณากรอกชื่อคลังสินค้า', 'error');
            nameField?.focus();
            return;
        }
        ...
        try {
            ...
            const data = wasCreate ? await createWarehouse(payload) : await updateWarehouse(payload);
            currentWarehouseId = String(data.warehouse_id);
            isCreateMode = false;
            ...
            populateForm(data);
            showMessage(
                wasCreate ? 'สร้างคลังสินค้าเรียบร้อยแล้ว' : 'บันทึกข้อมูลคลังสินค้าเรียบร้อยแล้ว',
                'success',
            );
            try {
                sessionStorage.setItem('warehouses:refresh', '1');
            } catch (storageError) {
            ...
            setDirtyState(true);
            if (error.message === 'warehouse_sn_exists') {
                showMessage('เลข Serial นี้ถูกใช้แล้ว กรุณาระบุใหม่', 'error');
            } else if (error.message === 'network') {
                showMessage('ไม่สามารถเชื่อมต่อกับเซิร์ฟเวอร์ได้ โปรดลองอีกครั้ง', 'error');
            } else {
                showMessage('ไม่สามารถบันทึกข้อมูลคลังสินค้าได้ โปรดลองอีกครั้ง', 'error');
            }
        } finally {
            isSaving = false;
            updateSaveButtonState();
    ...
    function attachEventListeners() {
        ...
        if (breadcrumbLink) {
            breadcrumbLink.addEventListener('click', (event) => {
                event.preventDefault();
                const href = breadcrumbLink.getAttribute('href') || './warehouses.html';
                requestNavigation(() => {
    ...
    const boot = ({ root }) => {
        ...
        if (isCreateMode) {
            ...
            loadWarehouse(currentWarehouseId);
        }
    };
